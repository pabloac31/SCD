...
void funcion_buffer()
{
   ...
   static int modo = 1;    // Modos de trabajo del buffer

   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
     int resto = modo==1 ? 1 : 0;
     bool flag = false;
      ...
      if(etiq_emisor_aceptable == MPI_ANY_TAG)
      {
        MPI_Probe( int MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &estado );
        if(estado.MPI_TAG == etiq_productor && estado.MPI_SOURCE % 2 != resto){
          flag = true;
        }

      }

      if(etiq_emisor_aceptable==etiq_productor || estado.MPI_TAG==etiq_productor )
        modo = modo==1 ? 2 : 1;

      if(flag)
        if(estado.MPI_TAG != etiq_consumidor && estado.MPI_SOURCE % 2 == resto)
