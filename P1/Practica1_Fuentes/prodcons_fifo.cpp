...
unsigned primera_ocupada = 0; // índice en el vector de la primera celda ocupada
...
void  funcion_hebra_productora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      int dato = producir_dato() ;
			...
			primera_libre = (primera_libre+1)%tam_vec;	// y actualizar posicion
...
void funcion_hebra_consumidora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      int dato ;
			sem_wait(ocupadas);
      dato = buffer[primera_ocupada];		// leer valor
			primera_ocupada = (primera_ocupada+1)%tam_vec; // actualizar posicion
			//cout << dato << " <- buffer" << endl; // para depurar
			sem_signal(libres);
      consumir_dato( dato ) ;
    }
